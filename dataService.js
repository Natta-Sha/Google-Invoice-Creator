...
/**
 * Delete invoice by ID: removes row, deletes Google Doc and PDF from Drive
 * @param {string} id - Invoice ID
...
function deleteInvoiceById(id) {
  try {
    const spreadsheet = getSpreadsheet(CONFIG.SPREADSHEET_ID);
    const sheet = spreadsheet.getSheetByName(CONFIG.SHEETS.INVOICES);
    const data = sheet.getDataRange().getValues();
    const headers = data[0];
    const idCol = headers.indexOf("ID");
    const docCol = headers.indexOf("Google Doc Link");
    const pdfCol = headers.indexOf("PDF Link");
    if (idCol === -1 || docCol === -1 || pdfCol === -1) {
      return { success: false, message: "Sheet structure error." };
    }
    const rowIdx = data.findIndex((row, i) => i > 0 && row[idCol] === id);
    if (rowIdx === -1) {
      return { success: false, message: "Invoice not found." };
    }
    const docUrl = data[rowIdx][docCol];
    const pdfUrl = data[rowIdx][pdfCol];
    // Delete Google Doc
    if (docUrl && docUrl.includes("/d/")) {
      const docId = docUrl.match(/\/d\/([\w-]+)/);
      if (docId && docId[1]) {
        try {
          DriveApp.getFileById(docId[1]).setTrashed(true);
        } catch (e) {
          return { success: false, message: "No previous document exists" };
        }
      }
    } else {
      return { success: false, message: "No previous document exists" };
    ...
    if (pdfUrl && pdfUrl.includes("/d/")) {
      const pdfId = pdfUrl.match(/\/d\/([\w-]+)/);
      if (pdfId && pdfId[1]) {
        try {
          DriveApp.getFileById(pdfId[1]).setTrashed(true);
        } catch (e) {
          return { success: false, message: "No previous PDF exists" };
        }
      }
    } else {
      // PDF is optional, so don't block deletion
    }
    // Delete row from sheet
    sheet.deleteRow(rowIdx + 1); // +1 for 1-based index
    return { success: true };
  } catch (error) {
