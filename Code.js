...
function getInvoiceList() {
  try {
    ...
    function formatDate(val) {
      ...
      // Если это объект Date (нормальный случай)
      ...
      // Если ISO-строка (2025-06-17T00:00:00Z)
      if (typeof val === "string" && val.includes("T")) {
        const [yyyy, mm, dd] = val.split("T")[0].split("-");
        return `${dd}/${mm}/${yyyy}`;
      }

      // Если строка в формате MM/DD/YYYY
      const mmddyyyy = val.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
      if (mmddyyyy) {
        const [_, mm, dd, yyyy] = mmddyyyy;
        return `${dd.padStart(2, "0")}/${mm.padStart(2, "0")}/${yyyy}`;
      }

      // Если уже правильный формат — оставим
